...
1. The button state is debounced using a timer based on `millis()`.
2. The LED toggles between ON and OFF.
3. A message is printed to the Serial Monitor.
4. Multiple false triggers caused by mechanical switch bounce are ignored.
...
* The button state is monitored continuously.
* A debounce timer is started whenever a change is detected.
* The new state is accepted only if it remains stable for 50 milliseconds.
...
* LED wired correctly with current-limiting resistor.
* Button connected using INPUT_PULLUP configuration.
* Debouncing implemented using millis().
* LED toggles correctly on button press.
* Serial Monitor displays one message per press.
* Pin assignments documented in config.h.
* Wiring photo saved in wiring-images/day-03.jpg.
* Sketch committed to branch day-03-debounce.
